...
const MovieInfo = ({ movie }) => {
    ...
    async function getTrailer() {
        if (pathname.includes("/movie") && movie) {
            ...
            if (response?.data?.results.length > 0) {
                ...
                movieTrailer(movie.title, { multi: true }).then((res) => {
                    setTrailer(res)
                });
            }
        }
    ...
    return movie && (
        <div style={{
            ...
            {/* <LazyLoadImage src={movie.poster} alt={movie.title} className='w-full hidden xl:inline-block h-100 object-cover' /> */}
            <div className="xl:bg-main bg-dry flex-colo xl:bg-opacity-90 xl:absolute top-0 left-0 right-0 bottom-0">
                <div className="container px-3 mx-auto 2xl:px-32 xl:grid grid-cols-3 flex-colo py-1 lg:py-20 gap-8">
                    <div className="xl:col-span-1 w-full h-header bg-dry border border-gray-800 rounded-lg overflow-hidden">
                        {(pathname.includes("movie") && trailer) ? <TrailerVideo movie={movie} trailer={trailer} /> : <LazyLoadImage src={movie.poster} className='max-h-96 object-cover mx-auto' alt={movie.title} />}
                    </div>
